...
import ai.bitlabs.sdk.data.model.sentry.SentryEnvelopeItemHeaders
...
internal class SentryRepository(
    ...
    fun sendEnvelope() {
        ...
        val event = gson.toJson(
            SentryEvent(
                eventId = evenId,
                timestamp = now,
                logentry = SentryMessage("Test message"),
                user = SentryUser(id = uid),
                sdk = SentrySDK(version = "0.1.0")
            )
        )

        val itemHeaders = gson.toJson(
            SentryEnvelopeItemHeaders(
                type = "event",
                length = event.length
            )
        )

        val envelopeHeaders = gson.toJson(
        ...
        val envelope = RequestBody.create(
        ...
        sentryAPI.sendEnvelope(envelope = envelope).enqueue(
            object : Callback<ResponseBody> {
                override fun onResponse(
                    ...
                    if (response.isSuccessful) {
                        Log.i(
                            TAG,
                            "onResponse: Sentry envelope sent. Response: ${
                        ...
                        Log.e(
                            TAG,
                            "onResponse: Sentry envelope not sent. Error: ${
